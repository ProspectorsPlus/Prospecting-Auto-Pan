...
> Releases are not published yet; this document describes the process that applies once they are. Until a public repository URL and releases exist, the only trustworthy source is building from the source tree yourself (`BUILDING.md`).
...
- Windows: `ProspectorLiteSetup.exe` (installer) and `ProspectorLite-portable-win64.zip` (portable)
...
```powershell
Get-FileHash .\ProspectorLiteSetup.exe -Algorithm SHA256
Get-FileHash .\ProspectorLite-portable-win64.zip -Algorithm SHA256
```
...
- **Version** — must match the release you downloaded (e.g. `1.0.0-rc.5`).
...
- **View Code**: when the build has a public repository URL configured, every View Code button opens the file **pinned to the build's commit** (`.../blob/<commit>/<file>#L<start>-L<end>` — `lite_trust.py`, `source_url_for`) — never a branch that could have moved since. When no URL is configured (the current state), the app says so and shows the exact local file + symbol + commit instead; it never silently falls back to a moving target.
