#!/usr/bin/env python3
"""Cross-language golden conformance suite. Part of the source tree (it
travels with source archives) but a DEV tool, never bundled into the
packaged apps. Its goldens live in a private sibling checkout that public
clones will not have, so the suite SELF-SKIPS with exit 0 when they are
absent -- expected on public clones and CI.

Runs every golden PPScript program through the REAL engine interpreter
(ScriptRunner in prospector_engine/engine.py) with a virtual clock, stubbed
input, and the shared IntervalScenario world, then compares the produced
trace against the expected trace generated by the reference VM.
