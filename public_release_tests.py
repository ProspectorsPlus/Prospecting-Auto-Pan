...
def scan_icons(files):
    """rc.4 gate additions: the diamond icon pipeline is complete and no
    stale pre-rebrand icon can ship. The OLD cowboy icon.png was 2048x2048 /
    4.4 MB (sha256 26e3b946...); the diamond assets are small, generated by
    packaging/make_icon.py, and both build pipelines regenerate their
    compiled icon EVERY build (the ! -f guards that once shipped a stale
    icon are banned)."""
    print("[icons]")
    import hashlib
    ...
    for size in (1024, 512, 256, 128, 64, 48, 32, 24, 16):
        ...
        chk(rel in files and os.path.isfile(os.path.join(ROOT, rel)),
            "%s present" % rel)
    chk(os.path.getsize(os.path.join(ROOT, "icon.png")) < 300 * 1024,
        "icon.png is the small generated diamond, not the 4.4MB legacy art")
    # both compiled-icon steps must be unconditional
    chk("! -f build/icon.icns" not in read("build_dmg.command"),
        "build_dmg.command has no stale-icns guard")
    chk("if not exist icon.ico" not in read("windows/build.bat"),
        "windows/build.bat has no stale-ico guard")
    # package definitions reference the icon
    chk("build/icon.icns" in read("prospector_lite_mac.spec"),
        "mac spec references build/icon.icns")
    chk('icon="icon.ico"' in read("windows/prospecting.spec")
        or "icon='icon.ico'" in read("windows/prospecting.spec"),
        "windows spec references icon.ico")
    chk("SetupIconFile=icon.ico" in read("windows/installer.iss"),
        "windows installer references icon.ico")
