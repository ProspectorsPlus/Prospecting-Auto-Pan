...
## D-099 — 2026-08-30 — `sample_box_px` set to 1: production now checks exactly what `--calibrate` shows
...
**Numbering note:** this entry and D-100 below were drafted independently as
D-094/D-095 in a separate worktree from the one that produced the D-094
(temporal bridge) through D-098 entries above, and both sides picked the same
two numbers for unrelated work. Renumbered to D-099/D-100 on merge; no
content changed.

---

## D-100 — 2026-08-30 — Fresh `--calibrate` values for every color-gated pixel, and `capacity_full` moved onto `color_close`

**Decision:** every color-gated field D-093/D-099 listed as owed a
re-measurement was re-derived with `--calibrate` under the new 1x1 semantics
and baked into `TreasurePixels`/`ChestPixel`:

| Field | Old (box-mean, stale) | New (1x1, D-100) |
|---|---|---|
