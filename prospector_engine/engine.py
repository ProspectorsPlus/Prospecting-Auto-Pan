...
# [Phase 04 C6] The engine lives in the prospector_engine/ package; its home
...
class _LagProbe:
    """Input-timing health probe: records how late the engine's timed sleeps
    actually WAKE. The felt smoothness of the macro is this number -- when a
    background thread hogs the GIL (big numpy / PNG / OCR-prep work), every
    click and hold in the shake wakes late and the cycle turns jittery.
    Counters only (no I/O in the hot path); reported with the 2s stats so
    live runs carry before/after evidence."""
    ...
    def note(self, late_ms):
        self.n += 1
        self.over_ms += late_ms
        if late_ms > self.max_ms:
            self.max_ms = late_ms
        if late_ms > self._alert:
            self._alert = late_ms

    def snapshot(self):
        return {"n": self.n,
                "mean_ms": round(self.over_ms / self.n, 2) if self.n else 0.0,
                "max_ms": round(self.max_ms, 1)}

    def pop_alert(self):
        a, self._alert = self._alert, 0.0
        if a > 20.0:
            return ("[lag] timed input sleeps waking late (worst %.0fms in "
                    "the last 2s) -- background load is fighting the input "
                    "thread" % a)
        return None
...
def _bg_qos_utility():
    """Drop the CALLING thread to macOS QoS UTILITY: analytics helpers prefer
    the efficiency cores and stop contending with the input-timing thread
    (and with the game/WindowServer). Best-effort, harmless elsewhere."""
    try:
        import ctypes
        ctypes.CDLL(None).pthread_set_qos_class_self_np(0x11, 0)
    except Exception:
        pass
    ...
    while True:
        ...
        if remaining > 0.002:
            ...
            while time.perf_counter() < deadline:
                pass
            break
    late = (time.perf_counter() - deadline) * 1000.0
    if late > 2.0:
        _LAG.note(late)
...
class FindsWatcher:
    ...
    def __init__(self):
        ...
        try:
            bm = float(FINDS_BAND_MIN)
        except Exception:
            bm = 0.0
        self._band_min = bm if bm >= 0.001 else 0.008
    ...
    def start(self):
        ...
        if self._band_min != FINDS_BAND_MIN:
            log("[finds] FINDS_BAND_MIN=%s makes every pixel row a card band "
                "(and burns CPU for nothing) -- using %.3f"
                % (FINDS_BAND_MIN, self._band_min))
        self._stop.clear()
        ...
        with mss.mss() as sct:
            while not self._stop.is_set() and (State.running or State.paused):
                ...
                try:
                    ...
                    sig_arr = arr[::2, ::2] if arr.shape[0] >= 800 else arr
                    bands = _finds_bands(_finds_row_signal(sig_arr),
                                         self._pitch, self._band_min)
    ...
    def _run_ocr(self):
        ...
        while not self._stop.is_set() and (State.running or State.paused):
            ...
            try:
                ts, arr, snap = fr
                # Retina captures are already 2x -- 2x more is all the text
                # needs (the old flat 3x made 6x-logical frames: ~2.3x the
            ...
            # duty ceiling: never run passes back-to-back -- a long pass must
            # leave the GIL (and a core) breathing room for input timing
            self._stop.wait(max(0.35 * dt, gap - dt))

    # ---- adaptive floors --------------------------------------------------
    def _mint_floor(self):
        f = self._band_min * 1.5
        if self._plateau > 0:
            f = max(f, 0.25 * self._plateau)
        return f

    def _leave_floor(self):
        f = self._band_min * 0.75
        if self._plateau > 0:
    ...
    def _feed_band(self, now, t, b):
        ...
        ref = max(self._plateau, t["peak"], self._band_min * 3.0)
...
def _finds_ocr_array(arr, factor=3):
    ...
    The pixels go to Vision as a CGImage built straight from the buffer: the
    old path PNG-ENCODED the upscaled frame and Vision then DECODED it again
    -- pure zlib busywork whose GIL holds measurably delayed the engine's
    timed input sleeps (the shake jitter). factor is the upscale (2 on Retina
    captures, which are already 2x; 3 on 1x captures).
    ...
    big = arr.repeat(factor, axis=0).repeat(factor, axis=1)
    hh, ww = h * factor, w * factor
    handler = None
    try:
        import Quartz
        prov = Quartz.CGDataProviderCreateWithCFData(big.tobytes())
        img = Quartz.CGImageCreate(
            ww, hh, 8, 24, ww * 3, Quartz.CGColorSpaceCreateDeviceRGB(),
            Quartz.kCGImageAlphaNone, prov, None, False,
        ...
        handler = None
    if handler is None:                    # fallback: the old PNG path
        import mss.tools
        from Foundation import NSData
        png = mss.tools.to_png(big.tobytes(), (ww, hh))
        data = NSData.dataWithBytes_length_(png, len(png))
        handler = Vision.VNImageRequestHandler.alloc().initWithData_options_(
            data, None)
    req = Vision.VNRecognizeTextRequest.alloc().init()
...
def _cli_main(argv=None):
    """Process entrypoint (the historical prospecting_old.py __main__ body).

    [Phase 04 C6] Called by the launcher shim (and the __main__ guard below);
    argv defaults to sys.argv[1:]. Behavior is unchanged by the package move.
    """
    global _IPC_MODE, _IPC_HOST, _IPC_PROTOCOL, _ENGINE_HOME, _SIM_MODE
    global CONFIG_FILE
    _args = sys.argv[1:] if argv is None else list(argv)
